...
class Requester():
    ...
    def send_message(self, config_path, message) -> None:
        ...
        if r.status_code == 200:
            self.log.success(
                f"Message sended. Date: {datetime.datetime.now()}")
        else:
            self.log.error(
                f"Error sending message. Date: {datetime.datetime.now()}")
    ...
    def requester(self, args, config_path, message):
        ...
        if args.sshd_pam_detection and platform.system() != "Linux":
            self.log.error_and_exit(
                "Pam flag enabled but this is not a Linux system. Skipping")
        elif "PAM_TYPE" in os.environ:
            if os.environ.get('PAM_TYPE') == "open_session":
                self.log.info(
                    "PAM enabled (Linux system) and PAM_TYPE = open_session")
